...
export default function RoomSection(props: RoomSectionProps) {
  ...
  const filteredEquipment = roomEquipment.filter((equipment: Equipment) =>
    equipment.name.toLowerCase().includes(props.equipmentSearch.toLowerCase())
  );
  ...
  const roomTrainings = props.room.trainingModules.map(
    moduleStatusMapper(currentUser.passedModules, currentUser.trainingHolds)
  );
      ...
      {roomTrainings.length > 0 && (
        <Stack direction={"column"} alignItems={isMobile ? "center" : "flex-start"} pl={"10px"} spacing={1}>
          ...
          <Stack direction={isMobile ? "column" : "row"} spacing={1} alignItems={"center"}>
            {roomTrainings.length > 0 && roomTrainings.map((ms: ModuleStatus) => <ModuleStatusRow ms={ms} />)}
          </Stack>
